...
class TestCaseCSV(TestCase):
    ...
    def test_process_line(self):
        ...
        gas = Category.objects.create(name="gas", titles="FILLER")
        ...
        # now ensure that titles is appended
        compare(gas.titles, "FILLER;;MCDONALDS")
...
class TestCaseViews(TestCase):
    ...
    def test_update_category_titles(self):
        old_cat = Category.objects.create(name="old_cat", titles="FILLER1;;FILLER2", total=0.00)
        new_cat = Category.objects.create(name="new_cat", titles="FILLER1;;FILLER2", total=4.00)

        # case: assigning li to new_cat
        li = LineItem.objects.create(name="test_li", category=old_cat, price=2.00, date='2018-04-06', recent=True)
        update_category_titles(li, old_cat, None)
        compare(old_cat.titles, "FILLER1;;FILLER2;;test_li")

        # case: changing li from old cat to new cat
        update_category_titles(li, old_cat, new_cat)
        compare(old_cat.titles, "FILLER1;;FILLER2")
        compare(new_cat.titles, "FILLER1;;FILLER2;;test_li")

    def test_update_category_price(self):
        old_cat = Category.objects.create(name="old_cat", titles="FILLER", total=0.00)
        new_cat = Category.objects.create(name="new_cat", titles="FILLER", total=4.00)

        # case: assigning li to new_cat
        li = LineItem.objects.create(name="test_li", category=old_cat, price=2.00, date='2018-04-06', recent=True)
        update_category_titles(li, old_cat, None)
        compare(old_cat.total, 2.00)


        # case: changing li from old cat to new cat
        update_category_titles(li, new_cat, old_cat)
        compare(old_cat.total, 0.00)
        compare(new_cat.total, 6.00)
