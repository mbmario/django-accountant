...
def assign_cat_to_li(request):
    ...
    try:
        ...
        new_cat = Category.objects.get(name=new_cat_name, user=user)
    except:
        return # can't trust jquery

    # update cat totals and titles
    new_cat.total = new_cat.total + li.price
    old_cat.total = old_cat.total - li.price
    ...
    li.category = Category.objects.get(name=new_cat_name, user=user)
    li.recent = False
    li.save()
